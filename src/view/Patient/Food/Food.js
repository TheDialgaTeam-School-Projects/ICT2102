...
export class FoodView extends Component {
  constructor(props) {
	...
	getBfast(data){
		...
		selectedButton = selectedButton ? selectedButton.value : this.state.bFastData[0].label;
		return selectedButton;
	}
	getLunch(data){
		let selectedButton = this.state.lunchData.find(e => e.selected == true);
		selectedButton = selectedButton ? selectedButton.value : this.state.lunchData[0].label;
		return selectedButton;
	}
	getDinner(data){
		let selectedButton = this.state.dinnerData.find(e => e.selected == true);
		selectedButton = selectedButton ? selectedButton.value : this.state.dinnerData[0].label;
		return selectedButton;
	}
